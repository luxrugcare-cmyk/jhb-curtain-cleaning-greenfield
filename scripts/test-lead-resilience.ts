...
async function main() {
  ...
  assert((crmDown.crm as { ok?: boolean }).ok === false, "CRM outage should be reported as failed");
  assert((crmDown.internalNotification as { ok?: boolean }).ok === true, "Internal notification should preserve durability during CRM outage");
  assert((crmDown.recovery as { archived?: boolean } | null)?.archived === true, "CRM outage should trigger recovery archive");
  assert(crmDown.warnings.some((warning) => warning.includes("crm: simulated Attio outage")), "CRM outage warning missing");
  ...
  assert(rejected, "Lead must reject when CRM, internal notification and recovery archive are all non-durable");
